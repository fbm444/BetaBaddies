...
if (process.env.LINKEDIN_CLIENT_ID && process.env.LINKEDIN_CLIENT_SECRET) {
  ...
  const linkedinStrategy = new LinkedInStrategy(
    ...
    async (accessToken, refreshToken, profile, done) => {
        ...
        console.log("📦 LinkedIn OAuth - Profile object:", profile ? "exists" : "null");
        ...
        if (!profile || (!profile.email && !profile.sub && !profile.id)) {
          console.log("⚠️ Passport profile is incomplete, will fetch manually...");
        ...
        try {
          ...
          if (!profile) {
            console.log("⚠️ Passport returned null profile, fetching manually from UserInfo endpoint...");
          } else {
            console.log("🔍 LinkedIn OAuth profile data:", JSON.stringify(profile, null, 2));
            console.log("🔍 LinkedIn OAuth _json:", JSON.stringify(profile?._json, null, 2));
          }
          ...
          try {
            ...
            console.log("✅ LinkedIn UserInfo data:", JSON.stringify(userInfo, null, 2));
            ...
            console.error("❌ Error fetching UserInfo:", userInfoError.response?.status, userInfoError.response?.data || userInfoError.message);
            console.error("❌ Full error:", JSON.stringify(userInfoError.response?.data || userInfoError.message, null, 2));
            ...
            if (userInfoError.response?.status === 401 || userInfoError.response?.status === 403) {
              return done(new Error(`LinkedIn authentication failed: Invalid or expired token. Status: ${userInfoError.response.status}`), null);
            }
            
            if (userInfoError.response?.status === 404) {
              // Try alternative endpoint
              console.log("⚠️ UserInfo endpoint not found, trying alternative...");
              try {
                ...
                console.log("✅ Alternative endpoint worked:", JSON.stringify(altResponse.data, null, 2));
                // Extract from alternative format
                linkedinId = altResponse.data.id || null;
                firstName = altResponse.data.firstName?.localized?.en_US || altResponse.data.firstName?.preferredLocale?.language || null;
                lastName = altResponse.data.lastName?.localized?.en_US || altResponse.data.lastName?.preferredLocale?.language || null;
            ...
            if (!email || !linkedinId) {
              ...
              linkedinId = profile?.sub || profile?.id || profile?._json?.sub || null;
            ...
            if (!email || !linkedinId) {
              return done(new Error(`Failed to fetch LinkedIn profile. UserInfo error: ${userInfoError.message}. Status: ${userInfoError.response?.status || 'unknown'}`), null);
            }
          }

          if (!email) {
            return done(new Error("LinkedIn profile does not include email address"), null);
          }
          ...
          if (user) {
            ...
            await userService.updateLinkedInTokens(user.u_id, accessToken, refreshToken);
            return done(null, { id: user.u_id, email: user.email });
          }
  ...
  linkedinStrategy.userProfile = async function(accessToken, done) {
    try {
      ...
      const userInfo = userInfoResponse.data;
      console.log("✅ LinkedIn UserInfo received:", JSON.stringify(userInfo, null, 2));
      ...
      console.error("❌ Error fetching UserInfo:", error.response?.status, error.response?.data || error.message);
      return done(new Error(`Failed to fetch LinkedIn profile: ${error.message}`), null);
    }
  };
