...
BEGIN
  ...
  -- Guarded by both table and column existence to avoid errors on environments
  -- where the auth/users table has a different primary key name.
  IF EXISTS (
    SELECT 1
...
-- Grant Permissions
GRANT SELECT, INSERT, UPDATE, DELETE ON follow_up_reminders TO ats_app_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON follow_up_reminder_rules TO ats_app_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON company_responsiveness_tracking TO ats_app_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON email_templates TO ats_app_user;
