...
if (!databaseUrl) {
  // Try to construct from Supabase-specific variables first, then fall back to generic
  const supabaseHost = process.env.SUPABASE_DB_HOST || process.env.DB_HOST;
  const supabasePort =
    process.env.SUPABASE_DB_PORT || process.env.DB_PORT || "6543";
  const supabaseName =
    process.env.SUPABASE_DB_NAME || process.env.DB_NAME || "postgres";
  const supabaseUser =
    process.env.SUPABASE_DB_USER || process.env.DB_USER || "postgres";
  const supabasePass = process.env.SUPABASE_DB_PASS || process.env.DB_PASS;

  if (!supabaseHost || !supabasePass) {
    console.error("❌ Error: Supabase database configuration is missing");
    ...
    console.error("\nOption 2: Individual Supabase variables");
    console.error("  SUPABASE_DB_HOST=aws-0-us-west-2.pooler.supabase.com");
    console.error("  SUPABASE_DB_PORT=6543");
    console.error("  SUPABASE_DB_NAME=postgres");
    console.error("  SUPABASE_DB_USER=postgres.jnjxvsncxfwencryoklc");
    console.error("  SUPABASE_DB_PASS=your-password");
    console.error(
      "\nNote: These are separate from local database variables (DB_HOST, DB_PORT, etc.)"
    );
    process.exit(1);
  }

  databaseUrl = `postgresql://${supabaseUser}:${supabasePass}@${supabaseHost}:${supabasePort}/${supabaseName}`;

  if (supabasePort === "6543" || databaseUrl.includes("pgbouncer")) {
...
let pgDumpCmd = "";
const pgDump17Path = "/opt/homebrew/opt/postgresql@17/bin/pg_dump";

if (existsSync(pgDump17Path)) {
  pgDumpCmd = pgDump17Path;
} else {
  try {
...
try {
  const version = execSync(`${pgDumpCmd} --version`, {
    encoding: "utf-8",
  }).trim();
  console.log(`✅ pg_dump found: ${version}\n`);
} catch (error) {
  console.error("❌ Error: pg_dump version check failed");
  process.exit(1);
}
...
try {
  // Remove query parameters that pg_dump doesn't understand (like pgbouncer=true)
  // pg_dump only needs: postgresql://user:pass@host:port/database
  const databaseUrlClean = databaseUrl.split("?")[0];
  ...
  execSync(
    `${pgDumpCmd} "${databaseUrlClean}" --no-owner --no-acl --clean --if-exists --format=plain --file="${outputFile}"`,
    { stdio: "inherit" }
  );
