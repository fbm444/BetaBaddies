...
class MarketIntelligenceService {
   ...
   * Store data in cache
   */
  async storeInCache(cacheKey, dataType, data, daysToExpire = 7, metadata = null) {
    try {
      await database.query(`
        ...
        DO UPDATE SET 
          ...
          updated_at = NOW()
      `, [cacheKey, dataType, JSON.stringify(data), metadata ? JSON.stringify(metadata) : null]);
    } catch (error) {
  ...
  async getSalaryIntelligence(jobTitle, location = null, industry = null) {
    ...
    try {
      ...
      if (yourData.sampleSize === 0 && location) {
        console.log(`📊 No salary data for ${jobTitle} in ${location}, trying without location filter...`);
      ...
      try {
        // Get salary estimate
        adzunaSalary = await adzunaService.getSalaryEstimate(jobTitle, [], location);
        
        console.log('📊 Adzuna data:', adzunaSalary ? `Median: $${adzunaSalary.median}` : 'N/A');
  ...
  async getYourSalaryData(jobTitle, location, industry) {
    ...
    return {
      average: Math.round((parseFloat(data.avg_min || 0) + parseFloat(data.avg_max || 0)) / 2) || 0,
      range: {
  ...
  generateSalaryComparison(yourData, marketData) {
    ...
    return {
      ...
      positioning: percentDiff > 10 ? 'above_market' : 
                   percentDiff < -10 ? 'below_market' : 'at_market',
      insight: this.generateSalaryInsight(percentDiff, difference, yourData.sampleSize)
    };
  }
  ...
  generateSalaryInsight(percentDiff, dollarDiff, sampleSize) {
    if (percentDiff > 10) {
      return `Your market shows salaries ${percentDiff}% above the national median ($${Math.abs(dollarDiff).toLocaleString()} higher). This could indicate strong local demand or more senior positions.`;
    } else if (percentDiff < -10) {
      return `Your market shows salaries ${Math.abs(percentDiff)}% below the national median ($${Math.abs(dollarDiff).toLocaleString()} lower). Consider targeting higher-paying opportunities or locations.`;
    } else {
      return `Your market aligns closely with the national median (within ${Math.abs(percentDiff)}%). This suggests competitive market positioning.`;
    }
  }
  ...
  async getIndustryTrends(industry, location = null, timeframe = 365) {
    ...
    try {
      ...
      const adzunaTrends = await adzunaService.getIndustryTrends(industry, location);
      const adzunaCompanies = await adzunaService.getTopCompanies(industry, location, 10);
      ...
      const result = {
        ...
        hiringActivity: adzunaTrends ? {
          ...
          hiringVelocity: 'high',
          insight: `${adzunaTrends.recentJobs} active job postings in the last 30 days. ${adzunaTrends.trend === 'growing' || adzunaTrends.trend === 'strong_growth' ? 'Strong hiring demand.' : 'Stable market conditions.'}`
        } : await this.calculateHiringActivity(industry, location, timeframe),
        topCompanies: adzunaCompanies.length > 0 ? adzunaCompanies : await this.getTopHiringCompanies(industry, location, timeframe),
  ...
  async calculateGrowthMetrics(industry, location, timeframe) {
    let query = `
      WITH periods AS (
        SELECT 
          ...
          COUNT(*) FILTER (WHERE created_at >= NOW() - INTERVAL '${timeframe * 2} days' 
    ...
    return {
      ...
      trend: data.growth_rate > 10 ? 'strong_growth' :
             data.growth_rate > 0 ? 'growing' :
             data.growth_rate < -10 ? 'declining' : 'stable'
    };
  }
  ...
  async calculateHiringActivity(industry, location, timeframe) {
    ...
    return {
      ...
      hiringVelocity: avgAge < 14 ? 'very_high' :
                      avgAge < 30 ? 'high' :
                      avgAge < 60 ? 'moderate' : 'low',
      insight: totalJobs === 0
  ...
  async getSkillDemandTrends(industry = null, timeframe = 365) {
    ...
    try {
      ...
      const result = {
        ...
        decliningSkills: [],
        source: skillsData.length > 0 ? 'Adzuna (Real-time job market)' : 'No data available',
  ...
  async extractTopSkills(industry, timeframe) {
    ...
    const skillsArray = Object.entries(skillCounts)
      .map(([skill, count]) => ({
        skillName: skill,
        demandCount: count,
        percentageOfJobs: totalJobs > 0 ? ((count / totalJobs) * 100).toFixed(1) : 0,
    ...
    try {
      ...
      if (this.openai) {
        try {
          ...
          console.warn('⚠️ AI insights failed, falling back to basic insights:', aiError.message);
          return await this.generateBasicInsights(userId, userProfile, marketData);
        }
      } else {
        return await this.generateBasicInsights(userId, userProfile, marketData);
      }
  ...
  async getMarketDataForUser(userProfile) {
    const location = userProfile.state ? `${userProfile.city}, ${userProfile.state}` : null;
    
    const [salaryData, industryTrends, skillTrends] = await Promise.all([
      this.getSalaryIntelligence(userProfile.job_title, location, userProfile.industry),
  ...
  async generateAIInsights(userId, userProfile, marketData) {
    const topSkillsWithDemand = marketData.skillTrends.topSkills.slice(0, 10).map(s => 
      `${s.skillName} (${s.demandCount.toLocaleString()} jobs, ${s.percentageOfJobs}%)`
    ).join(', ');
...
• Current Skills: ${userProfile.skills?.filter(s => s).join(', ') || 'Not specified'}

MARKET DATA:
• Your Market Salary: $${marketData.salaryData.yourMarket.average.toLocaleString()} (${marketData.salaryData.yourMarket.sampleSize} jobs)
• National Median: $${marketData.salaryData.nationalBenchmark?.median?.toLocaleString() || 'N/A'}
• Salary Positioning: ${marketData.salaryData.comparison?.percentDifference || '0'}% vs national
• Industry Growth: ${marketData.industryTrends.growthMetrics.growthRate}% (${marketData.industryTrends.growthMetrics.recentJobs.toLocaleString()} jobs, ${marketData.industryTrends.growthMetrics.trend})
• Hiring Velocity: ${marketData.industryTrends.hiringActivity.hiringVelocity}
• Top In-Demand Skills: ${topSkillsWithDemand}
• Top Hiring Companies: ${marketData.industryTrends.topCompanies.slice(0, 5).map(c => c.company).join(', ')}
  ...
  "insights": [{
    ...
    try {
      const response = await this.openai.chat.completions.create({
        model: 'gpt-3.5-turbo',
        messages: [
          { role: 'system', content: 'You are a career advisor providing data-driven insights.' },
  ...
  async generateBasicInsights(userId, userProfile, marketData) {
    ...
    if (marketData.salaryData.yourMarket.sampleSize > 0) {
      ...
      if (nationalMedian) {
        ...
        insights.push({
          ...
          actionableItems: comp.positioning === 'below_market' 
            ? ['Research higher-paying opportunities', 'Update resume to highlight achievements', 'Consider salary negotiation']
        ...
        insights.push({
          ...
          description: `Based on ${marketData.salaryData.yourMarket.sampleSize} jobs in your search, the average salary is $${yourAvg.toLocaleString()} with a range of $${marketData.salaryData.yourMarket.range.min.toLocaleString()} - $${marketData.salaryData.yourMarket.range.max.toLocaleString()}.`,
          actionableItems: ['Target positions in the higher end of this range', 'Highlight skills that command premium salaries', 'Research additional salary benchmarks'],
          supportingData: {
            average: yourAvg,
    ...
    if (growth.recentJobs > 0) {
      insights.push({
        ...
        description: `${userProfile.industry} shows ${growth.recentJobs} opportunities in the last ${marketData.industryTrends.timeframe} days${growth.growthRate !== 0 ? ` with ${growth.growthRate}% growth` : ''}. ${
          growth.trend === 'strong_growth' ? 'Strong hiring demand - favorable time to apply.' :
          growth.trend === 'declining' ? 'Slower hiring - be more selective.' :
          'Stable market conditions.'
        }`,
        actionableItems: growth.trend === 'strong_growth' || growth.recentJobs > 10
          ? ['Increase application volume', 'Act quickly on opportunities', 'Leverage strong demand in negotiations']
          : ['Focus on quality over quantity', 'Build stronger applications', 'Network actively'],
        supportingData: {
    ...
    if (marketData.skillTrends.topSkills.length > 0) {
      ...
      const highDemandSkills = marketData.skillTrends.topSkills
        .filter(s => s.demandCount > 1000 || parseFloat(s.percentageOfJobs) > 1)
        .slice(0, 5);
      ...
      if (highDemandSkills.length > 0) {
        insights.push({
          ...
          description: `The most sought-after skills are: ${highDemandSkills.map(s => `${s.skillName} (${s.demandCount.toLocaleString()} jobs)`).join(', ')}. Focus on these mainstream technologies when updating your materials.`,
          actionableItems: [
