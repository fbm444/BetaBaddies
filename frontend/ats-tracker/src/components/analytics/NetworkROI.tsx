...
export function NetworkROI({ dateRange }: NetworkROIProps) {
  ...
  const referralRate =
    data.overall.totalActivities > 0
      ? Math.round((data.overall.referrals / data.overall.totalActivities) * 100 * 10) / 10
      : 0;

  const opportunityRate =
    data.overall.totalActivities > 0
      ? Math.round(
          (data.overall.opportunitiesFromNetwork / data.overall.totalActivities) * 100 * 10
        ) / 10
      : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-2xl bg-white p-5 border border-[#E4E8F5]">
          <div className="flex items-start justify-between mb-2">
            <p className="text-[18px] font-normal text-[#0F1D3A]">Referrals Generated</p>
            <Icon icon="mingcute:user-add-line" width={20} className="text-[#09244B]" />
          </div>
          <p className="text-4xl font-extralight text-[#5A87E6]">{data.overall.referrals}</p>
          <p className="text-xs text-[#6D7A99] mt-1">{referralRate}% referral rate</p>
        </div>

        <div className="rounded-2xl bg-white p-5 border border-[#E4E8F5]">
          <div className="flex items-start justify-between mb-2">
            <p className="text-[18px] font-normal text-[#0F1D3A]">Opportunities</p>
            <Icon icon="mingcute:briefcase-line" width={20} className="text-[#09244B]" />
          </div>
          <p className="text-4xl font-extralight text-[#5A87E6]">
            {data.overall.opportunitiesFromNetwork}
          </p>
          <p className="text-xs text-[#6D7A99] mt-1">{opportunityRate}% opportunity rate</p>
        </div>

        <div className="rounded-2xl bg-white p-5 border border-[#E4E8F5]">
          <div className="flex items-start justify-between mb-2">
            <p className="text-[18px] font-normal text-[#0F1D3A]">Unique Contacts</p>
            <Icon icon="mingcute:contacts-line" width={20} className="text-[#09244B]" />
          </div>
          <p className="text-4xl font-extralight text-[#5A87E6]">{data.overall.uniqueContacts}</p>
        </div>
      </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="p-2 rounded-lg bg-white">
                      ...
                      <div className="flex items-baseline gap-1">
                        <p className="text-lg font-semibold text-[#3351FD]">{item.opportunities}</p>
      ...
      {data.overall.totalActivities === 0 && (
        <div className="rounded-2xl border border-dashed border-[#E4E8F5] bg-[#F8F8F8] p-10 text-center">
          <Icon icon="mingcute:user-community-line" className="mx-auto mb-3 text-[#6D7A99]" width={48} />
          <p className="text-sm text-[#6D7A99]">
            No networking activities tracked yet. Start networking to see ROI analytics.
          </p>
        </div>
      )}
    </div>
