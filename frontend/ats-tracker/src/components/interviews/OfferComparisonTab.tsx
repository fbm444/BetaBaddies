...
export function OfferComparisonTab({
  ...
  const fetchOffers = async () => {
    try {
      ...
      setError((err as Error).message || "Failed to fetch offers");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRemoveFromComparison = async (offerId: string) => {
    ...
    try {
      ...
      setError((err as Error).message || "Failed to remove offer");
    }
  };
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-200">
                ...
                {/* Only show equity row if at least one offer has equity */}
                {comparisonData.offers?.some((offer: any) => offer.equity_amount > 0 || offer.equity_type) && (
                  <tr>
                    <td className="px-6 py-4 text-sm font-medium text-slate-700 sticky left-0 bg-white">
                      Equity (4-yr)
                    </td>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">
                      Equity Amount (4-year value)
                    </label>
                    <input
                      type="number"
                      value={formData.equity_amount || ""}
                      onChange={(e) =>
                        setFormData({ ...formData, equity_amount: e.target.value ? parseFloat(e.target.value) : undefined })
                      }
                      placeholder="e.g., 100000"
                      className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      disabled={!formData.equity_type}
                    />
                    {formData.equity_type && (
                      <p className="text-xs text-slate-500 mt-1">
                        Estimated 4-year value of equity grant
                      </p>
                    )}
                  </div>
                </div>
